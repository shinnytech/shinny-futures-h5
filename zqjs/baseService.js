(function () {
    ...
    /**
     * main_ins_list: [], // 主力合约
     * SHFE_ins_list: [], // 上海期货交易所
     * CZCE_ins_list: [], // 郑州商品交易所
     * INE_ins_list: [], // 上期能源交易所
     * DCE_ins_list: [], // 大连商品交易所
     * CFFEX_ins_list: [], // 中国金融期货交易所
     */
    ...
    var getJsonDataAsync = function () {
        ...
        for(var symbol in result){
            if (result[symbol].expired){
                delete result[symbol];
                continue;
            }
            var item = result[symbol];
            if (item.class === 'FUTURE'){
                var ex = item.exchange_id;
                content[ex + '_ins_list'].push(symbol);
                ...
                content.main_ins_list.push(symbol);
                var ex = symbol.match(/@(.*)\./)[1];
                content[ex + '_ins_list'].push(symbol);
        ...
        for(var list_name in content){
            if(content[list_name] instanceof Array){
                content[list_name].sort(function(a, b){
                    var diff = result[a].sort_key - result[b].sort_key;
    ...
    function getInsIdById(insid) {
        ...
        } if(content_data[insid].class === 'FUTURE_CONT'){
            var match = insid.match(/@.*\.(.*)/)[1];
            return content_data[insid] ? match + '主连' : '';
        }else if( content_data[insid].class === 'FUTURE_INDEX') {
            var match = insid.match(/@.*\.(.*)/)[1];
            return content_data[insid] ? match + '指数' : '';
        }
    }
